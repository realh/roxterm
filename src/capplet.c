	...
	const char *name, gboolean dflt)
{
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(glade_xml_get_widget(glade,
				name)), options_lookup_int_with_default(options, name, dflt));
...
{
	char *value = 
		options_lookup_string_with_default(options, name, dflt ? dflt : "");

	capplet_ignore_changes = TRUE;
	gtk_entry_set_text(GTK_ENTRY(glade_xml_get_widget(glade, name)),
			value);
	capplet_ignore_changes = FALSE;
	g_free(value);
}
	...
	const char *name, int dflt)
{
	capplet_set_radio_by_index(glade, name, 
			options_lookup_int_with_default(options, name, dflt));
}
	...
	const char *name)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON
		(glade_xml_get_widget(glade, name)),
		options_lookup_double(options, name));
	capplet_ignore_changes = FALSE;
}
...
{
	double value = options_lookup_double(options, name);

	if (value == -1)
		value = dflt;
	capplet_ignore_changes = TRUE;
	gtk_range_set_value(GTK_RANGE(glade_xml_get_widget(glade, name)), value);
	capplet_ignore_changes = FALSE;
}
...
{
	capplet_ignore_changes = TRUE;
	gtk_combo_box_set_active(GTK_COMBO_BOX(glade_xml_get_widget(glade, name)),
		options_lookup_int_with_default(options, name, dflt));
	capplet_ignore_changes = FALSE;
}
...
void on_boolean_toggled(GtkToggleButton * button, Options * options)
{
	...
	if (capplet_ignore_changes)
		return;
	name = glade_get_widget_name(GTK_WIDGET(button));
	if (name)
	{
		...
		capplet_set_int(options, name, state);
	}
}
...
int capplet_which_radio_is_selected(GtkWidget *widget)
{
	...
	for (;list; list = g_slist_next(list))
	{
		...
		{
			const char *name = glade_get_widget_name(GTK_WIDGET(list->data));
...
void on_radio_toggled(GtkToggleButton * button, Options * options)
{
	...
	if (name)
	{
		...
		opt_name[l - 1] = 0;
		capplet_set_int(options, opt_name, n);
		g_free(opt_name);
	}
}
...
void on_float_spin_button_changed(GtkSpinButton *button, Options *options)
{
	...
	if (name)
	{
		capplet_set_float(options, name,
...
void on_float_range_changed(GtkRange * range, Options * options)
{
	...
	if (capplet_ignore_changes)
		return;
	name = glade_get_widget_name(GTK_WIDGET(range));
	if (name)
	{
		capplet_set_float(options, name, gtk_range_get_value(range));
	}
}
...
void on_combo_changed(GtkComboBox * combo, Options * options)
{
	...
	if (capplet_ignore_changes)
		return;
	name = glade_get_widget_name(GTK_WIDGET(combo));
	if (name)
	{
		capplet_set_int(options, name,
...
void on_file_chosen(GtkFileChooser * chooser, Options * options)
{
	...
	if (capplet_ignore_changes)
		return;
	name = glade_get_widget_name(GTK_WIDGET(chooser));
	if (name)
	{
		char *filename = gtk_file_chooser_get_filename(chooser);

		g_debug("File '%s' chosen for option '%s'", filename, name);
		capplet_set_string(options, name, filename);
		g_free(filename);
	}
}
#endif
