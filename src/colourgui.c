...
#define CG_CONNECT(cg, h) \
    glade_xml_signal_connect_data ((cg)->glade, #h, (GCallback) (h), (cg))
...
static void colourgui_set_colour_widget(GladeXML *glade,
		const char *widget_name, GdkColor *colour, gboolean ignore)
{
	...
	g_return_if_fail(gdk_colormap_alloc_color(colour_map, colour,
				TRUE, TRUE));
...
static void colourgui_set_fgbg_colour_widgets(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	colourgui_set_colour_widget(glade, "foreground_colour",
			colour_scheme_get_foreground_colour(colour_scheme, FALSE), ignore);
	colourgui_set_colour_widget(glade, "background_colour",
			colour_scheme_get_background_colour(colour_scheme, FALSE), ignore);
}

static void colourgui_set_cursor_colour_widget(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	colourgui_set_colour_widget(glade, "cursor_colour",
			colour_scheme_get_cursor_colour(colour_scheme, FALSE), ignore);
}

static void colourgui_set_all_palette_widgets(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	int n;
	...
	for (n = 0; n < 24; ++n)
	{
		...
		colourgui_set_colour_widget(glade, widget_name, &palette[n], ignore);
	}
}

static void colourgui_shade_row(GladeXML *glade, int row, gboolean sensitive)
{
	char widget_name[20];
	int column;
	...
	for (column = 0; column < 8; ++column)
	{
		...
		gtk_widget_set_sensitive(glade_xml_get_widget(glade,
				widget_name), sensitive);
	}
}

static void colourgui_set_palette_shading(GladeXML *glade)
{
	int size_radio = -1;
	...
	if (use_custom)
	{
		size_radio = capplet_which_radio_is_selected(
				glade_xml_get_widget(glade, "palette_size0"));
	}
	colourgui_shade_row(glade, 2, size_radio == 2);
	colourgui_shade_row(glade, 1, size_radio >= 1);
	colourgui_shade_row(glade, 0, size_radio >= 0);
	for (row = 0; row < 3; ++row)
	{
...
static void colourgui_set_fgbg_shading(GladeXML *glade)
{
	gboolean sensitive = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(glade_xml_get_widget(glade, "set_fgbg")))
...
static void colourgui_set_fgbg_track_shading(GladeXML *glade)
{
	gtk_widget_set_sensitive(glade_xml_get_widget(glade, "fgbg_track_palette"),
			gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
					glade_xml_get_widget(glade, "set_fgbg")))
			&& gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
					glade_xml_get_widget(glade, "use_custom_colours"))));
}

static void colourgui_set_cursor_shading(GladeXML *glade)
{
	gtk_widget_set_sensitive(glade_xml_get_widget(glade, "cursor_colour"),
			gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
					glade_xml_get_widget(glade, "set_cursor_colour"))));
}

static void colourgui_set_fgbg_toggle(GladeXML *glade, Options *colour_scheme,
		gboolean ignore)
{
...
static void colourgui_set_fgbg_track_toggle(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	...
	capplet_ignore_changes = ignore;
	gtk_toggle_button_set_active(
		GTK_TOGGLE_BUTTON(glade_xml_get_widget(glade, "fgbg_track_palette")),
		track);
	capplet_ignore_changes = FALSE;
}

static void colourgui_set_palette_toggles(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	int ps = colour_scheme_get_palette_size(colour_scheme); 
	GtkToggleButton *ucp =
		GTK_TOGGLE_BUTTON(glade_xml_get_widget(glade, "use_custom_colours"));
	...
	switch (ps)
	{
		...
		case 24:
			...
			capplet_set_radio_by_index(glade, "palette_size", ps / 8 - 1);
...
static void colourgui_set_cursor_toggle(GladeXML *glade,
		Options *colour_scheme, gboolean ignore)
{
	...
	capplet_ignore_changes = ignore;
	gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(glade_xml_get_widget(glade, "set_cursor_colour")),
			 cc != NULL);
	capplet_ignore_changes = FALSE;
	g_free(cc);
}

static void colourgui_set_all_toggles(GladeXML *glade, Options *colour_scheme,
		gboolean ignore)
{
	colourgui_set_fgbg_toggle(glade, colour_scheme, ignore);
	colourgui_set_fgbg_track_toggle(glade, colour_scheme, ignore);
	colourgui_set_palette_toggles(glade, colour_scheme, ignore);
	colourgui_set_cursor_toggle(glade, colour_scheme, ignore);
}

static void colourgui_set_all_shading(GladeXML *glade)
{
	colourgui_set_palette_shading(glade);
	colourgui_set_fgbg_shading(glade);
	colourgui_set_fgbg_track_shading(glade);
	colourgui_set_cursor_shading(glade);
}

static void colourgui_fill_in_dialog(ColourGUI * cg, gboolean ignore)
{
	GladeXML *glade = cg->glade;
	Options *colour_scheme = cg->colour_scheme;

	colourgui_set_all_toggles(glade, colour_scheme, ignore);
	colourgui_set_all_shading(glade);
	colourgui_set_all_palette_widgets(glade, colour_scheme, ignore);
	colourgui_set_fgbg_colour_widgets(glade, colour_scheme, ignore);
	colourgui_set_cursor_colour_widget(glade, colour_scheme, ignore);
}

static void colourgui_revert(ColourGUI *cg)
{
	options_copy_keyfile(cg->colour_scheme, cg->orig_scheme);
	colour_scheme_reset_cached_data(cg->colour_scheme);
...
static void on_Colour_Editor_destroy(GtkWidget *widget, ColourGUI * cg)
{
...
static void on_Colour_Editor_response(GtkWidget *widget, gint response,
		ColourGUI * cg)
{
	if (response == 1)
		colourgui_revert(cg);
...
static void on_Colour_Editor_close(GtkWidget *widget, ColourGUI * cg)
{
	colourgui_delete(cg);
}
...
{
	GtkColorButton *button = GTK_COLOR_BUTTON(glade_xml_get_widget(cg->glade,
				widget_name));
	GdkColor colour;
...
static void colourgui_set_colour_option_from_widget(ColourGUI *cg,
		...
		void (*set_fn)(Options *, const char *))
{
	colourgui_set_colour_option(cg->colour_scheme, opt_name,
...
static void on_color_set(GtkColorButton *button, ColourGUI * cg) 
{
	const char *widget_name;
	GdkColor colour;
	...
	char dyn_opt_nm[8];
	Options *opts = cg->colour_scheme;

	if (capplet_ignore_changes)
	{
		return;
	}
	widget_name = glade_get_widget_name(GTK_WIDGET(button));
	...
	else if (!strncmp(widget_name, "palette", 7) && strlen(widget_name) == 10)
	{
		...
		if (row == 0 && gtk_toggle_button_get_active(
						GTK_TOGGLE_BUTTON(glade_xml_get_widget(cg->glade,
								"fgbg_track_palette"))))
		{
			if (column == 7)
			{
				colourgui_set_colour_widget(cg->glade,
						"background_colour", &colour, TRUE);
				capplet_set_string(cg->colour_scheme,
	...
	if (option_name)
	{
		capplet_set_string(cg->colour_scheme, option_name, colour_name);
	}
	else
	{
...
static void on_set_fgbg_toggled(GtkToggleButton *button, ColourGUI * cg)
{
	gboolean state;
	...
	colourgui_set_fgbg_shading(cg->glade);
	colourgui_set_fgbg_track_shading(cg->glade);
	colourgui_set_fgbg_track_toggle(cg->glade, cg->colour_scheme, FALSE);
	...
	{
		COLOURGUI_SET_COLOUR_OPTION(cg->colour_scheme, foreground, NULL);
		COLOURGUI_SET_COLOUR_OPTION(cg->colour_scheme, background, NULL);
	}
}
		...
		const char *target_widget, const char *target_option)
{
	...
	gtk_color_button_get_color(
		GTK_COLOR_BUTTON(glade_xml_get_widget(cg->glade, source_widget_name)),
		&colour);
	colour_name = colourgui_name_colour(&colour);
	colourgui_set_colour_widget(cg->glade, target_widget, &colour, TRUE);
	capplet_set_string(cg->colour_scheme, target_option, colour_name);
}

static void on_fgbg_track_palette_toggled(GtkToggleButton *button,
		ColourGUI * cg)
{
	gboolean state;
		...
		ColourGUI *cg)
{
	...
	state = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(glade_xml_get_widget(cg->glade,
					"use_custom_colours")));
	old_size = colour_scheme_get_palette_size(cg->colour_scheme);
	if (state)
	{
		palette_size = capplet_which_radio_is_selected(
				glade_xml_get_widget(cg->glade, "palette_size0")) * 8 + 8;
	}
	else
	{
		palette_size = 0;
	...
	if (palette_size != old_size)
	{
		...
		colour_scheme_set_palette_size(cg->colour_scheme, palette_size);
		if (!palette_size)
		{
			gtk_toggle_button_set_active(
						GTK_TOGGLE_BUTTON(glade_xml_get_widget(cg->glade,
		...
		colourgui_set_fgbg_track_shading(cg->glade);
		colourgui_set_palette_shading(cg->glade);
		for (n = old_size; n < palette_size; ++n)
		{
			...
			sprintf(key, "%d", n);
			val = options_lookup_string(cg->colour_scheme, key);
			if (!val)
			{
				char widget_name[20];
				const char *colour_name;
				...
				colour_scheme_set_palette_entry(cg->colour_scheme, n,
						colour_name);
				capplet_set_string(cg->colour_scheme, key, colour_name);
			}
			else
			{
				g_free(val);
		...
		capplet_set_int(cg->colour_scheme, "palette_size", palette_size);
	}
}

static void on_set_cursor_colour_toggled(GtkToggleButton *button, ColourGUI *cg)
{
	int state;
	...
	colourgui_set_cursor_shading(cg->glade);
	state = gtk_toggle_button_get_active(button);
	if (state)
		COLOURGUI_SET_COLOUR_OPTION_FROM_WIDGET(cg, cursor);
	else
		COLOURGUI_SET_COLOUR_OPTION(cg->colour_scheme, cursor, NULL);
}

static void on_palette_size_toggled(GtkToggleButton *button,
		ColourGUI *cg)
{
...
static void colourgui_connect_handlers(ColourGUI * cg)
{
	/* Misc */
	CG_CONNECT(cg, on_Colour_Editor_destroy);
	CG_CONNECT(cg, on_Colour_Editor_close);
	CG_CONNECT(cg, on_Colour_Editor_response);

	/* Generic colour selection handler*/
	CG_CONNECT(cg, on_color_set);

	/* Widgets */
	CG_CONNECT(cg, on_set_fgbg_toggled);
	CG_CONNECT(cg, on_palette_size_toggled);
	CG_CONNECT(cg, on_fgbg_track_palette_toggled);
	CG_CONNECT(cg, on_set_cursor_colour_toggled);
	CG_CONNECT(cg, on_use_custom_colours_toggled);
}
...
ColourGUI *colourgui_open(const char *scheme_name, GdkScreen *scrn)
{
	...
	cg->colour_scheme = colour_scheme_lookup_and_ref(scheme_name);
	cg->orig_scheme = options_copy(cg->colour_scheme);

	cg->glade = glade_xml_new(capplet_get_glade_filename(),
		"Colour_Scheme_Editor", NULL);
	g_assert(cg->glade);
	cg->widget = glade_xml_get_widget(cg->glade, "Colour_Scheme_Editor");
    if (scrn)
        gtk_window_set_screen(GTK_WINDOW(cg->widget), scrn);
	...
	if (!cg->orig_scheme)
	{
		...
		gtk_widget_set_sensitive(glade_xml_get_widget(cg->glade, "revert"),
				FALSE);
	}
...
void colourgui_delete(ColourGUI * cg)
{
	...
	UNREF_LOG(colour_scheme_unref(cg->colour_scheme));
	g_free(cg);
	capplet_dec_windows();
