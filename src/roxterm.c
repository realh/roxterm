                                  ...
                                  ROXTermData *roxterm)
{
    ...
    MultiWin *win = roxterm_get_win(roxterm);
    guchar *clipboard_content = NULL;
    gsize clipboard_len = 0;
    ...
    switch (roxterm->allow_osc52)
    {
        ...
        case 1:
            if (text_len)
            {
                clipboard_content = g_malloc(text_len);
                memcpy(clipboard_content, text, text_len);
                clipboard_len = text_len;
            }
            else
            {
                /* Signifies we want to clear the clipboard when the button is
                 * pressed */
                clipboard_len = 1;
            }
            break;
        case 2:
        case 3:
            multi_win_write_clipboard(win, text, text_len, primary);
            break;
    }
    
    multi_win_flash_clipboard_indicator(win, roxterm->allow_osc52 != 3,
                                        clipboard_content, clipboard_len,
                                        primary);
}
