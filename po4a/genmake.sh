...
do
    ...
    printf >>$f '\tpo4a-gettextize -f docbook -m $< -p $@\n\n'
done
...
do
    ...
    do
        ...
        printf >>$f '\tpo4a-translate -k 0 -f docbook -m ../%s.1.xml.in -p $< -l $@\n\n' $r
        echo >>$f $r.1.$l.po: ../$r.1.xml.in
        printf >>$f '\tpo4a-updatepo -f docbook -m $< -p $@\n\n'
    done
done
...
do
    ...
    printf >>$f '\tpo4a-gettextize -f xhtml -m $< -p $@\n'
    echo >>$f
    for l in $LANGS
    do
        echo >>$f $p.html.$l.po: ../Help/en/$p.html
        printf >>$f '\tpo4a-updatepo -f xhtml -m $< -p $@\n'
        echo >>$f
        echo >>$f ../Help/$l/$p.html: $p.html.$l.po
        printf >>$f '\tpo4a-translate -k 0 -f xhtml -m ../Help/en/%s.html -p $< -l $@\n\n' $p
    done
done
